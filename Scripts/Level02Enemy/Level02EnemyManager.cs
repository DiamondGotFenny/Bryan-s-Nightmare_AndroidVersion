...
public class Level02EnemyManager : MonoBehaviour {
    ...
    void SpawnCDControl()
    {
        ...
        {    
            ...
            if (hourGlass0.Length==4||hourGlass0.Length+hourglass.currentHGNumber >= 4)
            {
                var clones = GameObject.FindGameObjectsWithTag("Enemy");
                foreach (var enemyclone in clones)
                {
                    if (enemyclone.name == "StrongZambie(Clone)")
                    {
